...
class _BettingScreenState extends State<BettingScreen> {
  ...
  void initState() {
    ...
    // Kiểm tra balance và hiển thị dialog nạp tiền nếu <= 0
  ...
  void _showTopUpDialog({String? message}) {
    if (message != null) {
    ...
    showDialog(
      ...
      builder: (context) => TopUpDialog(
        currentBalance: _gameState.balance,
        onTopUp: (amount) {
          setState(() {
            _gameState = _gameState.copyWith(
              balance: _gameState.balance + amount,
            );
          });
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Đã nạp ${amount.toStringAsFixed(0)} vào tài khoản!',
              ),
  ...
  void _startRace() {
    ...
    if (totalBet > _gameState.balance) {
      // Hiển thị dialog nạp tiền thay vì chỉ SnackBar
      _showTopUpDialog(
        message: 'Tổng tiền cược vượt quá số dư! Vui lòng nạp thêm tiền.',
      );
      return;
    }
